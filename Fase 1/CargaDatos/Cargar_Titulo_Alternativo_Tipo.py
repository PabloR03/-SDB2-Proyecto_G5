...
class OptimizedTituloAlternativoTipoLoader:
    def __init__(self):
        self.conn_params = {
            'host': 'localhost',
            'port': 5432,
    ...
    def check_tipos_exist_batch(self, id_tipo_list: List[int]) -> Set[int]:
        """Verificar qué id_tipo existen en la tabla Tipo_Titulo (en lotes)"""
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("""
                    SELECT id_tipo 
                    FROM imdb_schema.Tipo_Titulo 
                    WHERE id_tipo = ANY(%s)
                """, (id_tipo_list,))
            ...
            logger.error(f"Error verificando tipos de título existentes: {e}")
            raise
    
    def clean_titulo_alternativo_tipo_table(self):
        """Limpiar la tabla Titulo_Alternativo_Tipo antes de cargar"""
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("DELETE FROM imdb_schema.Titulo_Alternativo_Tipo")
                self.conn.commit()
                logger.info("Tabla Titulo_Alternativo_Tipo limpiada")
        except Exception as e:
            logger.error(f"Error limpiando tabla Titulo_Alternativo_Tipo: {e}")
            self.conn.rollback()
            raise
        ...
        try:
            ...
            for i in range(0, len(valid_records), self.insert_batch_size):
                ...
                with self.conn.cursor() as cursor:
                    execute_values(
                        cursor,
                        """INSERT INTO imdb_schema.Titulo_Alternativo_Tipo 
                    ...
                    if total_inserted % 10000 == 0:
                        logger.info(f"Insertados {total_inserted:,} registros de título alternativo-tipo...")
    ...
    def process_csv_chunk(self, df_chunk: pd.DataFrame) -> Tuple[int, int, int]:
        ...
        id_tipo_list = df_chunk['id_tipo'].dropna().astype(int).unique().tolist()
        ...
        existing_tipos = self.check_tipos_exist_batch(id_tipo_list)
        
        # Filtrar registros válidos
        valid_records = []
        invalid_titulo_alternativo_count = 0
        invalid_tipo_count = 0
        
        for _, row in df_chunk.iterrows():
            id_titulo_alternativo_tipo = int(row['id_titulo_alternativo_tipo'])
            ...
            # Verificar si id_tipo existe en Tipo_Titulo
            if id_tipo not in existing_tipos:
                invalid_tipo_count += 1
                continue
            
            # Si llegamos aquí, el registro es válido
            valid_records.append((id_titulo_alternativo_tipo, id_titulo_alternativo, id_tipo))
        
        # Insertar registros válidos de este chunk
        if valid_records:
            self.insert_valid_records_batch(valid_records)
        
        return len(valid_records), invalid_titulo_alternativo_count, invalid_tipo_count
    ...
    def process_csv_optimized(self, csv_path: str, clean_table: bool = True):
        ...
        try:
            ...
            if clean_table:
                logger.info("Limpiando tabla Titulo_Alternativo_Tipo...")
                self.clean_titulo_alternativo_tipo_table()
            ...
            for chunk_num, df_chunk in enumerate(chunk_iter, 1):
                ...
                # Procesar chunk
                valid_count, invalid_titulo_alternativo, invalid_tipo = self.process_csv_chunk(df_chunk)
                
                # Actualizar contadores
                total_valid += valid_count
                total_invalid_titulo_alternativo += invalid_titulo_alternativo
                total_invalid_tipo += invalid_tipo
                ...
                total_invalid = invalid_titulo_alternativo + invalid_tipo
            ...
            total_invalid_all = total_invalid_titulo_alternativo + total_invalid_tipo
            
            print("\n" + "="*75)
            print("RESUMEN DEL PROCESO OPTIMIZADO - TITULO_ALTERNATIVO_TIPO")
            print("="*75)
            ...
            print(f"Registros omitidos por id_tipo inválido: {total_invalid_tipo:,}")
    ...
    def get_final_count(self) -> int:
        ...
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("SELECT COUNT(*) FROM imdb_schema.Titulo_Alternativo_Tipo")
...
def main():
    # Configuración
    CSV_PATH = "/home/manuelimal02/Descargas/titulo_alternativo_tipo.csv"
    
    print("Iniciando carga optimizada de Titulo_Alternativo_Tipo...")
    print(f"Archivo: {CSV_PATH}")

    loader = OptimizedTituloAlternativoTipoLoader()
    
    try:
        # Procesar el archivo CSV de forma optimizada
        loader.process_csv_optimized(CSV_PATH, clean_table=True)
        print("\nProceso de Titulo_Alternativo_Tipo completado exitosamente!")
