...
class OptimizedProduccionDirectorLoader:
    def __init__(self):
        self.conn_params = {
            'host': 'localhost',
            'port': 5432,
        ...
        self.conn = None
        self.batch_size = 10000  # Procesar de 10k en 10k
        self.insert_batch_size = 1000  # Insertar de 1k en 1k
    ...
    def clean_produccion_director_table(self):
        """Limpiar la tabla Produccion_Director antes de cargar"""
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("DELETE FROM imdb_schema.Produccion_Director")
                self.conn.commit()
                logger.info("Tabla Produccion_Director limpiada")
        except Exception as e:
            logger.error(f"Error limpiando tabla Produccion_Director: {e}")
            self.conn.rollback()
            raise
        ...
        try:
            ...
            for i in range(0, len(valid_records), self.insert_batch_size):
                ...
                with self.conn.cursor() as cursor:
                    execute_values(
                        cursor,
                        "INSERT INTO imdb_schema.Produccion_Director (id_produccion_director, id_persona, id_produccion) VALUES %s",
                        batch,
                        page_size=500
                    )
                    self.conn.commit()
                    ...
                    if total_inserted % 10000 == 0:
                        logger.info(f"Insertados {total_inserted:,} registros de producción-director...")
    ...
    def process_csv_chunk(self, df_chunk: pd.DataFrame) -> Tuple[int, int, int]:
        ...
        for _, row in df_chunk.iterrows():
            id_produccion_director = int(row['id_produccion_director'])
            ...
            valid_records.append((id_produccion_director, id_persona, id_produccion))
    ...
    def process_csv_optimized(self, csv_path: str, clean_table: bool = True):
        ...
        try:
            ...
            if clean_table:
                logger.info("Limpiando tabla Produccion_Director...")
                self.clean_produccion_director_table()
            ...
            print("\n" + "="*75)
            print("RESUMEN DEL PROCESO OPTIMIZADO - PRODUCCION_DIRECTOR")
            print("="*75)
    ...
    def get_final_count(self) -> int:
        ...
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("SELECT COUNT(*) FROM imdb_schema.Produccion_Director")
...
def main():
    # Configuración
    CSV_PATH = "/home/manuelimal02/Descargas/produccion_director.csv" 
    
    print("Iniciando carga optimizada de Produccion_Director...")
    print(f"Archivo: {CSV_PATH}")

    # Crear instancia del loader optimizado
    loader = OptimizedProduccionDirectorLoader()
    
    try:
        # Procesar el archivo CSV de forma optimizada
        loader.process_csv_optimized(CSV_PATH, clean_table=True)
        print("\nProceso de Produccion_Director completado exitosamente!")
