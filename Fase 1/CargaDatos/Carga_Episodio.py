...
class OptimizedEpisodioLoader:
    ...
    def clean_episodio_table(self):
        """Limpiar la tabla Episodio antes de cargar"""
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("DELETE FROM imdb_schema.Episodio")
                self.conn.commit()
                logger.info("Tabla Episodio limpiada")
        except Exception as e:
            logger.error(f"Error limpiando tabla Episodio: {e}")
            self.conn.rollback()
            raise
        ...
        try:
            ...
            for i in range(0, len(valid_records), self.insert_batch_size):
                ...
                with self.conn.cursor() as cursor:
                    execute_values(
                        cursor,
                        "INSERT INTO imdb_schema.Episodio (tconst, id_titulo, temporada, episodio) VALUES %s",
                        batch,
                        page_size=500
                    )
                    self.conn.commit()
                    ...
                    if total_inserted % 10000 == 0:
                        logger.info(f"Insertados {total_inserted:,} episodios...")
    ...
    def process_csv_chunk(self, df_chunk: pd.DataFrame) -> Tuple[int, int, int]:
        ...
        # Extraer todos los tconst únicos del chunk (tanto tconst como id_titulo)
        tconst_episodio = df_chunk['tconst'].dropna().unique().tolist()
        id_titulo_serie = df_chunk['id_titulo'].dropna().unique().tolist()
        
        # Combinar ambas listas y eliminar duplicados
        all_tconst = list(set(tconst_episodio + id_titulo_serie))
        
        # Verificar cuáles existen en la tabla Titulo
        existing_tconst = self.check_titulos_exist_batch(all_tconst)
        
        # Filtrar registros válidos
        valid_records = []
        invalid_tconst_count = 0
        invalid_id_titulo_count = 0
        
        for _, row in df_chunk.iterrows():
            tconst = row['tconst']
            id_titulo = row['id_titulo'] if pd.notna(row['id_titulo']) else None
            temporada = int(row['temporada']) if pd.notna(row['temporada']) else None
            episodio_num = int(row['episodio']) if pd.notna(row['episodio']) else None
            ...
            # Verificar si id_titulo existe en Titulo (si no es None)
            if id_titulo is not None and id_titulo not in existing_tconst:
                invalid_id_titulo_count += 1
                continue
            
            # Si llegamos aquí, el registro es válido
            valid_records.append((tconst, id_titulo, temporada, episodio_num))
        
        # Insertar registros válidos de este chunk
        if valid_records:
            self.insert_valid_records_batch(valid_records)
        
        total_invalid = invalid_tconst_count + invalid_id_titulo_count
        return len(valid_records), invalid_tconst_count, invalid_id_titulo_count
    ...
    def process_csv_optimized(self, csv_path: str, clean_table: bool = True):
        ...
        try:
            ...
            if clean_table:
                logger.info("Limpiando tabla Episodio...")
                self.clean_episodio_table()
            ...
            for chunk_num, df_chunk in enumerate(chunk_iter, 1):
                ...
                # Procesar chunk
                valid_count, invalid_tconst, invalid_id_titulo = self.process_csv_chunk(df_chunk)
                
                # Actualizar contadores
                total_valid += valid_count
                total_invalid_tconst += invalid_tconst
                total_invalid_id_titulo += invalid_id_titulo
                ...
                total_invalid = invalid_tconst + invalid_id_titulo
            ...
            total_invalid_all = total_invalid_tconst + total_invalid_id_titulo
            
            print("\n" + "="*70)
            print("RESUMEN DEL PROCESO OPTIMIZADO - EPISODIOS")
            print("="*70)
            ...
            print(f"Registros omitidos por id_titulo inválido: {total_invalid_id_titulo:,}")
    ...
    def get_final_count(self) -> int:
        ...
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("SELECT COUNT(*) FROM imdb_schema.Episodio")
...
def main():
    # Configuración
    CSV_PATH = "/home/manuelimal02/Descargas/episodio.csv" 
    
    print("Iniciando carga optimizada de episodios...")
    print(f"Archivo: {CSV_PATH}")

    # Crear instancia del loader optimizado
    loader = OptimizedEpisodioLoader()
    
    try:
        loader.process_csv_optimized(CSV_PATH, clean_table=True)
        print("\n¡Proceso de episodios completado exitosamente!")
